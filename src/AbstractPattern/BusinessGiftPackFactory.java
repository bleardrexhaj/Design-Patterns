...
public class BusinessGiftPackFactory extends GiftPackFactory{
    static GiftPackFactory buildGiftPack(List<GiftItem> giftItems) {
        GiftPackFactory giftPack = new GiftPackFactory();
        System.out.println("The customer places an order of type Business Package");
        List<GiftItem> items = new ArrayList<GiftItem>();
        for (GiftItem giftItem : giftItems) {
            GiftItem newGiftItem = null;
            switch (giftItem.getPackagingType()) {
                case "Bag":
                    newGiftItem = new BagGiftItem(0.50f);
                    break;

                case "Box":
                    newGiftItem = new BoxGiftItem(1.00f);
                    break;

                case "Wrap":
                    newGiftItem = new WrapGiftItem(0.25f);
                    break;
